...
public:
    CIndex ( const T_ & seq , const C_ & cmp = C_ () ):  sequence ( seq ) , comparator ( cmp ) { }
    ~CIndex( void) { }
    ...
    set<size_t> search ( const T_ & pattern ) {
        ...
        int iSeq = 0;
        auto seqI = sequence.begin();
        int jPat = 0;
        auto patJ = pattern.begin();

        if (pattern.empty()) {
            for (auto iter = sequence.begin(); iter != sequence.end(); iter++, iSeq++)
                result.insert( iSeq );
            return result;
        }
        ...
        while (iSeq < seqSize) {
            if ((!comparator(*patJ, *seqI) && (!comparator(*seqI, *patJ))) == true) {
                patJ++;
                seqI++;
                jPat++;
                iSeq++;
            ...
            if (jPat == patSize) {
                result.insert((iSeq - jPat));
                jPat = lps[jPat - 1];
                patJ = next(pattern.begin(), jPat);
            }
            else if (iSeq < seqSize && ((!comparator(*patJ, *seqI) && (!(comparator(*seqI, *patJ)))) == false)) {
                if (jPat != 0) {
                    jPat = lps[jPat - 1];
                    patJ = next(pattern.begin(), jPat);
    ...
    void computeLPSArray(const T_ & pat, int patSize, int* lps)
    {
        int j = 0;
        auto patJ = pat.begin();
        ...
        lps[0] = 0;
        int i = 1;
        auto patI = next ( pat.begin(), 1 );
        while (i < patSize) {
            if ( (! comparator (* patI, *patJ ) && ( ! comparator ( *patJ , *patI) ) ) == true )  {
                j++;
                patJ++;
                lps[i] = j;
                i++;
